const experiencias = [
  ...
  },
  {
    ...
    texto:
      "Participei como estagiário no programa de capacitação Crescer. O objetivo desse programa é capacitar e inserir novos talentos da área de tecnologia no mercado. Durante minha participação no Crescer, adquiri experiência em diversas áreas, incluindo: </br> Desenvolvimento de aplicações front-end utilizando React. </br>  Criação de APIs  EST utilizando Spring e .NET 7.0. </br>  Realização de testes unitários. </br>  Elaboração de documentação de APIs com Swagger. </br>  Aplicação de modelos arquiteturais como Onion Architecture. </br>  Utilização de Docker para a criação de contêineres de aplicações. </br>  Automação de fluxos de trabalho através do Github Actions. </br>  Trabalho com bancos de dados PostgreSQL e SQLite. </br>  Utilização do Git, GitLab e GitHub para versionamento de código. </br>  Além disso, tive a oportunidade de aprender e aplicar metodologias ágeis, como o Kanban e Scrum.",
    tecnologias: [
      "Métodologias ageis",
      "Responsabilidade",
      "Proatividade",
